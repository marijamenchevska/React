...
const popularCountries = ['France', 'United States', 'Italy', 'Spain', 'China', 'Thailand', 'Australia', 'Egypt', 'Japan', 'Mexico'];
...
export default function MainComponent ({countries}: MainComponentProps) {
    const popularCountriesObjects = countries.filter(country => popularCountries.includes(country.name.common));

    const [searchWord, setSearchWord] = useState<string>('');
    const [foundCountries, setFoundCountries] = useState<Country[]>([]);

    useEffect (() => setFoundCountries(
        countries.filter(country => country.name.common.toLowerCase().startsWith(searchWord.toLowerCase()) === true)
    ), [searchWord]);
    ...
    return (
        <div className="py-5 shadow-md">
            ...
            {!searchWord && <PopularCountries popularCountries={popularCountriesObjects} />}
        </div>
    );
}